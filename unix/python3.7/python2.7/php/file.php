...
/*  PHP文件系统处理
//
//  1.概述,文件类型@type，文件属性@pro；
//  2.目录路径@path解析, 遍历，统计个数大小；
//  3.文件操作函数@func，开关，内容读写，锁定机制；
//  4.文件上传，设置，处理，多个上传，上传类，
//  5.文件下载
//  
//
 *	所有文件处理都是使用系统函数完成的。
...
echo dirname($url1).'<br>';             // ./aaa/bbb
echo dirname(dirname($url1)).'<br>';    // ./aaa
echo "<pre>";                           // filename:index
    print_r(pathinfo($url3));           // filename;basename;dirname;extension
echo "</pre>";

// 4.file operate func:           handle
...
touch("./apache.php");
unlink("c:/appserv/www/apache.php");    //del;
rename("./test.txt", "c:/test2.txt");   //move,newname
 ...
 */
date_default_timezone_set("PRC");
date("Y-m-d H:i:s",filemtime($fileName))/time();
stat($fileName);
pow(2,40);
